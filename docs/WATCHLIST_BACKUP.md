...
在合并分支或更新版本后，关注池（观测池）中的数据可能会丢失。这是因为数据库文件（*.db）被 `.gitignore` 排除在版本控制之外，导致在分支合并或版本更新时数据库无法自动保留。

## 解决方案

我们实现了全面的自动备份、恢复和持久化功能，确保您的关注池数据永远不会丢失：
- ✅ **自动备份**: 每次修改观测池时自动创建备份
- ✅ **自动恢复**: 系统启动时自动检测并从最新备份恢复数据
- ✅ **版本持久化**: 备份文件纳入版本控制，跨版本更新保留数据
- ✅ **手动管理**: 支持手动导出/导入备份文件
...
**日志示例**：
```
INFO: Watchlist is empty, checking for backups to restore...
INFO: Found backup: watchlist_backup_20260209_143000.json, attempting to restore...
INFO: ✅ Successfully restored 5 items from watchlist_backup_20260209_143000.json
```
...
**2026-02-09**
- ✅ 添加自动恢复功能 - 系统启动时自动从最新备份恢复数据
- ✅ 备份文件纳入版本控制 - 确保跨版本更新时数据持久化
- ✅ 创建 `data/backups/.gitkeep` - 确保备份目录被版本控制跟踪
