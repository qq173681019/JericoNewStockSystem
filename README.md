...
系统实现了完善的数据持久化机制，确保您的观测池和历史记录永不丢失：

#### ✅ 自动备份
- **触发时机**: 每次添加或删除观测池股票时自动创建备份
- **存储位置**: `data/backups/` 目录
- **文件格式**: JSON格式，易于阅读和编辑
- **文件命名**: `watchlist_backup_YYYYMMDD_HHMMSS.json`

#### ✅ 自动恢复
- **启动检查**: 系统启动时自动检查数据库状态
- **智能恢复**: 如果观测池为空且存在备份文件，自动从最新备份恢复
- **版本更新**: 更新代码后数据自动恢复，无需手动操作
...
# 导入备份（合并模式）
python backup_watchlist.py import backup.json

# 列出所有备份
python backup_watchlist.py list
```

详细文档请参阅：[关注池备份与恢复功能](docs/WATCHLIST_BACKUP.md)
