...
**后端实现** (run_web_ui.py):
- `predict_stock()` 函数: 检测实时数据获取失败，返回错误消息
- `predict_multi_timeframe()` 函数: 检测历史数据获取失败，返回错误消息
- 错误响应包含：success=False, error类型, 详细的中文提示信息

**前端实现** (app.js):
- `displayErrorMessage()` 函数显示用户友好的错误卡片
- `runPrediction()` 函数在预测失败时调用错误显示
...
**代码位置**: DatabaseManager.__init__() 方法中调用
